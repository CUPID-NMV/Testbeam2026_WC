...
SingleModuleDetector::SingleModuleDetector()
: G4VUserDetectorConstruction() {}

SingleModuleDetector::~SingleModuleDetector() {}

G4VPhysicalVolume* SingleModuleDetector::Construct() {
  ...
  // Acqua deionizzata: proprieta' ottiche realistiche (da Testbeam2026_WC)
  ...
  // Teflon (PTFE): riflettore Lambertian, riflettivita' energy-dependent (da Testbeam2026_WC)
  ...
  // =======================================================
  // --- PARAMETRI GEOMETRIA FIBRE E PMT ---
  // =======================================================
  const G4double x_st1           = 380.0 * mm;
  ...
  for (int f = 0; f < 37; f++) {
      ...
      // 5. Bundle verticale in HDPE (dentro il foro, nel World)
      if (lBunHDPE) {
...
void SingleModuleDetector::ConstructSDandField() {
    // PMT_SD_L / PMT_SD_R: mappati a pmtID 0 e 1 in PMTSD.cc
