...
!!! info "Limitations"
    * Only supports periodic, polled, and manually-triggered equipment (no interrupt-triggered option).
...
def parse_args():
    ...
    * -O to become a daemon but retain stdout
    ...
    the global `parser` object and call add_argument() etc. 
    ...
    Returns:
        (Namespace): The result of calling parser.parse_args()
    """
...
class EquipmentBase:
    ...
    * **Manually-triggered** equipment (`midas.EQ_MANUAL_TRIG`) - The frontend will 
        wait for a webpage or code to trigger an event (via RPC), and then 
        call `readout_func()`.
    ...
    * `poll_func()` if you are creating a polled equipment
    ...
    Attributes:
        ...
        is_manual_trig (bool): If you're in readout_func(), whether this call was
            from a MANUAL_TRIG RPC call, or a regular periodic/polled trigger
    ...
    def _odb_common_callback(self, client, path, new_value):
        ...
        Parameters:
            client (midas.client.MidasClient): Needed to match the expected interface
                of odb_watch callbacks
            path (str): Needed to match the expected interface of odb_watch callbacks
    ...
    def _is_periodic(self):
        """
        Whether this is a periodic equipment.
    ...
    def _is_polled(self):
        """
        Whether this is a polled equipment.
    ...
    def _is_manual_trig(self):
        """
        Whether this is a manually-triggered equipment.
        
        Returns:
            (bool):
        """
        return self.common["Type"] & midas.EQ_MANUAL_TRIG
    ...
    def _get_events(self):
        ...
        Parameters:
            is_manual_trig (bool): If this was from MANUAL_TRIG RPC calls
        
        Returns:
            (list of midas.event.Event): Events that user generated (or an empty list
                if they didn't generate any this time)
        """
...
class FrontendBase:
    ...
    * Create and add any equipment.
    ...
    def __init__(self, frontend_name):
        ...
        Parameters:
            frontend_name (str): The name of this client/frontend. It should be
        ...
        cb = midas.callbacks.make_manual_trig_rpc_callback(self._manual_trigger_callback, self)
        self.client.lib.c_cm_register_function(midas.RPC_MANUAL_TRIG, cb)
        ...
        while True:
            ...
            for equip in self._enabled_equipment():
                if equip._is_active_for_state(self.run_state):
                    ...
                    ev_id = equip.common["Event ID"]
                    if equip._is_manual_trig() and ev_id in self.manual_triggers and self.manual_triggers[ev_id]:
                        # Manual trigger has been requested
                        self._get_and_send_events(equip, True)
                        self.manual_triggers[ev_id] = False
            ...
            self.client.check_deferred_transition()
    ...
    def _get_and_send_events(self, equip, is_manual_trig=False):
        """
        ...
        Parameters:
            equip (EquipmentBase):
            is_manual_trig (bool): whether this was triggered by a MANUAL_TRIG RPC call
        """
        equip.is_manual_trig = is_manual_trig
        events = equip._get_events()
        equip.is_manual_trig = False
    ...
    def _tr_start_callback(self, client, run_number):
        ...
        Parameters:
            client (midas.client.MidasClient): Needed to match the expected interface
                of register_transition_callback callbacks
            run_number (int): The new run number
        """
        for equip in self.equipment.values():
            equip._reset_stats()
    ...
    def _frontend_exit_callback(self, client):
        ...
        Parameters:
            client (midas.client.MidasClient): Needed to match the expected interface
                of register_disconnect_callback callbacks
        """
        self.frontend_exit()

    def _manual_trigger_callback(self, client, event_id):
        """
        Callback function we register to be called for RPC_MANUAL_TRIG requests.
        
        Parameters:
            client (midas.client.MidasClient): Needed to match the expected interface
                of register_disconnect_callback callbacks
            event_id (int): Event ID the user wants to trigger
        """
        self.manual_triggers[event_id] = True
        return midas.status_codes["SUCCESS"]
