...
This file contains definitions that come from midas.h, plus some helper classes.

Classes:
    MidasLogHandler: helps you integrate midas messages and python logging
    MidasError: base class of exceptions that can get raised. The midas python interface
        doesn't use status codes when functions return, but raises exceptions instead.
    MidasLib: contains the low-level tools for interacting with the midas C++ code. Users
        almost certainly don't need to know how it works (or even that it exists...).
"""
...
See [`midas.MidasLogHandler`][] for details of how to set up logging.
"""
...
RPC_MANUAL_TRIG = 17000 # RPC code for manually-triggering frontends
...
"""All midas status codes, as a dict"""
...
if have_numpy:
    # Optimal mapping from midas type to numpy data type
...
def sys_is_big_endian():
    """
    Get whether this machine is big-endian.
...
def get_np_dtype(midas_tid):
    """
    Get the optimal numpy dtype for a given midas data type.

    Parameters:
        midas_tid (int): one of midas.TID_xxx

    Returns:
        (np.dtype): numpy datatype
    """
...
class WaitTimeoutExpired(Exception):
    """
    Exception type raised when the sequencer wait_xxx() functions time out.
    """
    pass
...
class MidasLogHandler(logging.Handler):
    ...
    def emit(self, record):
        """
        Implement the emit function needed by base logging.Handler.
        ...
        # Use standard formatting from base logging.Handler
...
def status_code_to_text(code):
    ...
    Parameters:
        code (int): status code from `midas.status_codes`
...
def safe_to_json(input_str, use_ordered_dict=False):
    ...
    Parameters:
        input_str (str): JSON
        use_ordered_dict (bool): Whether to preserve the order of keys in the
