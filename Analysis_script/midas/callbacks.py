...
# return BOOL (start transition now?); args int (transition type) / BOOL (first time being called?)
...
def make_deferred_transition_callback(callback, client):
    ...
    Python function (`callback`) details:
        * Arguments it should accept:
            * client (midas.client.MidasClient)
            transition_type (int): `midas.TR_STOP`, `midas.TR_START` etc
        * Value it should return:
    ...
    def _wrapper(transition_type, dummy):
        try:
            retval = callback(client, transition_type)
        except Exception as e:
            traceback.print_exc()
...
def make_manual_trig_rpc_callback(callback, client):
    """
    Create a callback function that can be passed to cm_register_function from the midas
    C library.
    ...
    Python function (`callback`) details:
        * Arguments it should accept:
            * client (midas.client.MidasClient)
            * event_id (int): The equipment event ID the user wants to trigger
        * Value it should return:
            * Anything or nothing
    """
    ...
    # As with the other make_xxx_callback functions, we create a closure.
    def _wrapper(index, params):
        event_id_p = ctypes.cast(params[0], ctypes.POINTER(ctypes.c_int))
        event_id = event_id_p.contents.value

        try:
            retval = callback(client, event_id)
        except Exception as e:
            traceback.print_exc()
            retval = midas.status_codes["FE_ERR_DRIVER"]
