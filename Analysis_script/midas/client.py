...
Classes:
    MidasClient: The main class that most users will interact with.
  
Attributes:
    have_numpy (bool): Whether numpy is available.
    logger (logging.Logger): Python logger for this module. See [`midas.MidasLogHandler`][] for more.
"""
...
class MidasClient:
    ...
    the main [`midas`][] module (e.g. `midas.TID_WORD`), and optionally the 
    [`midas.frontend`][] and [`midas.sequencer`][] modules.
    
    Most common midas functions have wrappers defined in this class. For 
    example, the [`odb_set`][] function allows you to pass normal python values as
    arguments:
    ...
    ??? info "Recipe for adding support for more functions"
        ...
        * Define an appropriate C-compatible function in `include/midas_c_compat.h`
            and `src/midas_c_compat.cxx` (e.g. `c_my_func()`).
        * Recompile using cmake (`cd $MIDASSYS/build && make && make install`)
        * Call the function from python like `client.lib.c_my_func()`. You will need 
            to convert any arguments to ctypes values. If the C function 
            doesn't return an integer, then there is even more work to do - see the
            [`midas.MidasLib`][] class for details.
    ...
    def odb_chmod(self, path, mode, recurse=True):
        """
        Set permissions on an ODB key, and optionally all sub-keys if `path`
        is a directory.

        The default permissions of new ODB keys have read/write/delete enabled.

        !!! example
            To allow read/write, but not delete:

            ```
            client.odb_chmod("/Some/path", midas.MODE_READ|midas.MODE_WRITE)
            ```

            which is equivalent to

            ```
            client.odb_chmod("/Some/path", 3)
            ```
        
        Parameters:
            path (str): e.g. "/Runinfo/State"
            mode (int): Combination of midas.MODE_READ/midas.MODE_WRITE/midas.MODE_DELETE to allow
            recurse (bool): Recurse into sub-keys if path is a directory
        """          
        path = self._odb_strip_final_slash(path)  
        hKey = self._odb_get_hkey(path, False)
        c_mode = ctypes.c_uint16(mode)
        c_recurse = ctypes.c_uint32(recurse)
        self.lib.c_db_set_mode(self.hDB, hKey, c_mode, c_recurse)
    ...
    def odb_delete(self, path, follow_links=False):
        ...
        self.lib.c_db_delete_key(self.hDB, hKey)
    ...
    def odb_get(self, path, recurse_dir=True, include_key_metadata=False, just_key_list=False):
        ...
        Info: A note on dict ordering
            Midas directories have an associated order to them
        ...
        Returns:
            (dict): If `path` points to a directory and/or `include_key_metadata`
                is True. Dicts are keyed by the
            ...
            (int/float/bool etc, or a list of them): If `path` points to a single 
                ODB key and `include_key_metadata` is False.
            (list of strings): If `just_key_list` is True, and `path` points to a directory.
    ...
    def odb_watch(self, path, callback, pass_changed_value_only=False):
        ...
        Info: Callback function details if `pass_changed_value_only` is FALSE
        ...
        Info: Callback function details if `pass_changed_value_only` is TRUE
            * Arguments it should accept:
                * client (midas.client.MidasClient)
        ...
        !!! example
            See `examples/basic_client.py` for more.
    ...
    def register_transition_callback(self, transition, sequence, callback):
        ...
        Info: Callback function details
            * Arguments it should accept:
                * client (midas.client.MidasClient)
    ...
    def register_deferred_transition_callback(self, transition, callback):
        ...
        Info: Callback function details
            * Arguments it should accept:
                * client (midas.client.MidasClient)
                * transition_type (int): The transition that has been requested (e.g. `midas.TR_STOP`)
            * Value it should return:
    ...
    def check_deferred_transition(self):
        """
        If we're currently in a deferred transition, call the user's callback
        function to see if the work has been completed. If so, proceed with
        the actual transition.

        If you are writing a program that registers a deferred transition, but
        is NOT using the frontend framework, you must call this function 
        repeatedly in your main loop. In the frontend framework it is called
        for you automatically.
        """
        self.lib.c_cm_check_deferred_transition()
    ...
    def client_exists(self, client_name, exact_match=True):
        ...
        Note that this function does not consider the client that is
        performing the request (i.e. if program "mytest" asks if "mytest"
        is running, this function will return False!).
        ...
        Returns:
            (bool): Whether this client is running
        """
        c_name = ctypes.create_string_buffer(bytes(client_name, "utf-8"))
        c_unique = ctypes.c_uint32(exact_match)
    ...
    def clients_exist(self, client_names, return_as_dict=False):
        ...
        Returns:
            (dict of {str: bool}): If return_as_dict is True: whether each
                client is running or not.
            (boolean): If return_as_dict is False: whether all the clients/
                programs are running or not. If _any_ client/program is not running,
                we'll return False.
        """
    ...
    def connect_to_other_client(self, other_client_name):
        ...
        Parameters:
            other_client_name (str): name of the other client (e.g. as shown in the Programs webpage)
            
        Returns:
            (int): handle that you should pass to [`disconnect_from_other_client`][] and
                [`jrpc_client_call`][].
        """
    ...
    def brpc_client_call(self, connection, cmd, args, as_event=False, use_numpy=False, max_len=1000000):
        ...
        Raises:
            midas.MidasError: if the other client returns a non-SUCCESS status
                
        Returns:
            (ctypes.Array of ctypes.char_c): the data populated by the other client if `as_event` is False.
            (midas.event.Event): the data populated by the other client if `as_event` is True.
        """
        c_cmd = ctypes.create_string_buffer(bytes(cmd, "utf-8"))
        c_args = ctypes.create_string_buffer(bytes(args, "utf-8"))
        c_buf = ctypes.create_string_buffer(max_len)
    ...
    def manual_trig_client_call(self, connection, event_id):
        """
        Call the MANUAL_TRIG function exposed by another client.
        
        Parameters:
            connection (int): The connection handle returned to you when 
                connecting with `connect_to_other_client()`.
            event_id (int): Equipment event ID to trigger on other client.
        """
        self.lib.c_manualtrig_rpc_client_call(connection, event_id)
    ...
    def register_brpc_callback(self, callback):
        ...
        Note:
            * The javascript RPC call must take the format `mjsonrpc_call("brpc", params, "arraybuffer")`
            * `midas.event.Events` may be decoded in javascript with the `bkToObj` function defined in [midas.js](https://bitbucket.org/tmidas/midas/src/develop/resources/midas.js)
        """
    ...
    def reset_alarm(self, alarm_name):
        ...
        Parameters:
            alarm_name (str): The name of the alarm to reset. If empty, will 
                reset all alarms.
        """
    ...
    def get_recent_messages(self, min_messages=1, before=None, facility="midas"):
        ...
        Info: The number of messages returned is based on:
    ...
    def hist_get_tags(self, event_name):
        ...
        Returns:
            (list of dicts): like `[{"name": string, "type": int, "n_data": int}]`
                where 
    ...
    def hist_get_recent_data(self, num_hours, interval_secs, event_name, tag_name, index=None, timestamps_as_datetime=False):
        ...
        For all other parameters, see [`hist_get_data`][]

        Returns:
            (list of dicts): see [`hist_get_data`][]
        """
    ...
    def hist_get_data(self, start_time, end_time, interval_secs, event_name, tag_name, index=None, timestamps_as_datetime=False):
        ...
        Parameters:
            ...
            event_name (str): Name of the midas history "event" to query. Use
                [`hist_get_events`][] to find valid event names.
            tag_name (str): Name of the midas history "tag" to query. Use
                [`hist_get_tags`][] to find valid tag names for a given event.
        ...
        Returns:
            (list of dicts): One dict per event/tag/index combination. The dict
    ...
    def hist_create_plot(self, group_name, panel_name, variables, labels=[]):
        ...
        Arguments:
            ...
            variables (list of str): The variables to show, as referred to by the
        ...
        Raises:
            ...
            ValueError: if there is a problem with the other arguments
            TypeError: if there is a problem with the other arguments
        """
    ...
    def _odb_path_is_for_single_array_element(self, path):
        ...
        Returns:
            (bool): Whether it's for a single element
            (int/None): The array index
            (ctypes string buffer): The path without the array index
        """
    ...
    def _odb_get_value(self, path, recurse_dir=False, array_single=False, 
        ...
        Returns:
            (anything): If include_key_metadata is False.
            (dict): If include_key_metadata is True.
        """
        c_path = ctypes.create_string_buffer(bytes(path, "utf-8"))
