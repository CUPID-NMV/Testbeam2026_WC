...
# TODO - Want a way to stop sequencer if we're in a long-running function in a library.
...
class SequenceClient(midas.client.MidasClient):
    ...
    define in your script (`define_params()`, `sequence()` and `at_exit()`).
    ...
    !!! example "An example sequencer script"
        ```
    ...
    def get_param(self, name):
        ...
        Parameters:
            name (str): Name of a parameter previously created by [`register_param`][]
    ...
    def wait_func(self, func, check_period_secs=0.1, timeout_secs=None):
        """
        Wait until an arbitrary condition is met. 
        
        The function you specify should return either:

        * a boolean - True if the sequencer should continue, or False if we should 
        wait and try again.
        * a 4-tuple of (boolean, string, number, number) for (wait_is_complete, text, 
        current_value, target_value). `wait_is_complete` has the same meaning as the "single boolean"
        return value. `text` is a short string to show the user what we're waiting for.
        `current_value` and `target_value` are used to draw the progress bar. These values
        only really make sense if you're counting "up" to something. If `current_value` is
        larger than `target_value`, the progress bar will show 100%. You might want to 
        manipulate your values so they show absolute differences.
        ...
        Parameters:
            ...
            timeout_secs (int/float/None): Raise a `midas.WaitTimeoutExpired` exception 
                if the condition hasn't been met within the specified period. None means
                wait indefinitely.

        !!! example "Example using a lambda (anonymous one-line function)"
            ```
            def sequence(seq):
                seq.wait_func(lambda : seq.odb_get("/path/aaa") > 1.5 and seq.odb_get("/path/bbb") < 3.11)
            ```

        !!! example "Example using a custom function (which must return a boolean or 4-tuple)"
            ```
            import random

            def custom_func():
                # This version returns a 4-tuple
                target = 7
                ...
                return (complete, message, val, target)
        ...
        !!! example "Example using a custom function, with a timeout"
            ```
            import random
            import midas
            # Note we had to import midas, so we can specify midas.WaitTimeoutExpired
            # in our exception handling!

            def custom_func():
                # This version just returns a boolean
                target = 7
                val = random.randint(0, 10)
                print(f"random val was {val}")
            ...
            def sequence(seq):
                ...
                # that we should call to check if the condition is met or not.
                try:
                    seq.wait_func(custom_func, timeout_secs=0.5)
                except midas.WaitTimeoutExpired:
                    seq.sequencer_msg("Wait timeout expired...")
            ```
        """
        ...
        while True:
            ...
            if isinstance(ret_value, (list, tuple)):
                (complete, message, curr_value, target_value) = ret_value
            else:
                complete = ret_value
                message = "Function"
                curr_value = 0
            ...
            waiting_secs = (datetime.datetime.now() - wait_start_time).total_seconds()
            comms.state_queue.put(WaitInfo(message, curr_value, target_value, timeout_limit=timeout_secs, timeout_value=waiting_secs))

            if timeout_secs is not None and waiting_secs > timeout_secs:
        ...
        if timeout_expired:
            raise midas.WaitTimeoutExpired(f"Timeout expired waiting for custom function to complete")
        
    def wait_odb(self, path, op, target, between_upper_target=None, stable_for_n_secs=None, timeout_secs=None):
        """
        ...
        !!! example "Example with basic comparison"
            ```
            def sequence(seq):
                seq.wait_odb("/Runinfo/Run number", ">", 12345)
            ```

        !!! example "Example with a range and a timeout"
            ```
            import midas
            # Note we had to import midas, so we can specify midas.WaitTimeoutExpired
            # in our exception handling!
            ...
            def sequence(seq):
                try:
                    seq.wait_odb("/Runinfo/Run number", "between", 12345, 56789, timeout_secs=5)
                    seq.sequencer_msg("Wait was successful!")
                except midas.WaitTimeoutExpired:
                    seq.sequencer_msg("Wait timeout expired...")
            ```
        """
        ...
        while True:
            ...
            if valid:
                ...
                comms.state_queue.put(WaitInfo("ODBValueStability", delta_secs, stable_for_n_secs, odb_path=path, odb_op=op, started_ms=started_ms, timeout_limit=timeout_secs, timeout_value=waiting_secs))
            else:
                # We're not valid - need to reset the clock if we're checking for stability
                stable_start_time = None
                comms.state_queue.put(WaitInfo("ODBValue", curr_value, target, odb_path=path, odb_op=op, timeout_limit=timeout_secs, timeout_value=waiting_secs))
        ...
        if timeout_expired:
            raise midas.WaitTimeoutExpired(f"Timeout expired waiting for '{path}' to reach {target}")
    ...
    def wait_events(self, target, timeout_secs=None):
        """
        ...
        while True:
            ...
            waiting_secs = (datetime.datetime.now() - wait_start_time).total_seconds()
            comms.state_queue.put(WaitInfo("Events", curr_value, target, timeout_limit=timeout_secs, timeout_value=waiting_secs))
        ...
        if timeout_expired:
            raise midas.WaitTimeoutExpired(f"Timeout expired waiting for event sent to reach {target}")
    ...
    def wait_clients_running(self, client_names, timeout_secs=10):
        ...
        Parameters:
            ...
            timeout_secs (float/int): how long to wait before giving up and concluding
                ...
                you previously called `start_client()` etc. Raises a `midas.WaitTimeoutExpired`
                exception if the timeout fails. 

        !!! warning ""   
            Note that a timeout is REQUIRED in this function, unlike the other
            wait_xxx functions where it is optional (allowing them to wait indefinitely).
        ...
        !!! example "Example"
            ...
            def sequence(seq):
                try:
                    seq.wait_clients_running(["Logger", "my_fe"])
                except midas.WaitTimeoutExpired:
                    seq.sequencer_msg("Wait timeout expired...")
            ```
        """
        ...
        while True:
            now = datetime.datetime.now()
            waiting_secs = (now - start).total_seconds()
            remaining_secs = timeout_secs - waiting_secs
            ...
            num_missing = len(missing_clients)

            if num_missing == 0:
                break
            
            num_wanted = len(client_names)
            num_running = num_wanted - num_missing
            comms.state_queue.put(WaitInfo("ClientsRunning", num_running, num_wanted, timeout_limit=timeout_secs, timeout_value=waiting_secs))

            if remaining_secs <= 0:
                break

            time.sleep(min(remaining_secs, 0.1))

        comms.state_queue.put(WaitInfo("", 0, 0))

        if len(missing_clients) > 0:
            raise midas.WaitTimeoutExpired(f"Timeout waiting for programs {missing_clients} to be running")
    ...
    def disable_tracing(self):
        ...
        !!! example
            ```
            # The webpage will just show the state at `do_abc()` and `do_xyz()`, skipping all
    ...
    def enable_tracing(self):
        """
        Opposite of [`disable_tracing`][].
        """
        global _user_tracing_enabled
        _user_tracing_enabled = True

    def keep_modules_when_reloading_script(self, module_names):
        """
        Some python modules don't like being imported multiple times.
        This can be a problem for sequencer scripts, as we generally want
        to re-import modules when starting a script, so that any changes a
        user has made to their own libraries get picked up automatically
        (e.g. if the user has a collection of helper functions or analysis
        tools).

        Known problematic modules are ones that use Rust code for their
        implementation, and **PyO3** to interface with python. You may see a message
        like "ImportError: PyO3 modules compiled for CPython 3.8 or older may 
        only be initialized once per interpreter process".

        !!! example "sshtunnel module"
            A concrete example is use of the `sshtunnel` package, which uses `paramiko`,
            which uses `cryptography`. And the cryptography Rust bindings are the issue.
            To fix this, we can use this line in our sequencer script (either in
            `define_params()` or `sequence()`):

            ```
            seq.keep_modules_when_reloading_script("cryptography.hazmat.bindings._rust")
            ```

        !!! example "Using a regex"
            If you have a large number of modules/submodules to keep, you can pass a
            compiled regular expression. Any modules that match the regex will be kept.
            You can also pass an array of regexes/strings to this function, e.g:

            ```
            # Minor annoyance in documetation - your code should only have
            # a SINGLE backslash, not two.
            modules = []
            modules.append(re.compile(r"numpy\\.random.*"))
            modules.append("bisect")
            seq.keep_modules_when_reloading_script(modules)

            # This would match "numpy.random", "numpy.random.xyz", "bisect".
            # This would not match "xyz.numpy.random", "_bisect", "bisect.xyz".
            ```

        Parameters:
            module_names (str, `re.Pattern`, or list of such): modules names to keep
        """
        if not isinstance(module_names, list):
            module_names = [module_names]

        comms.state_queue.put(KeepModuleInfo(module_names))
    ...
    def communicate(self, *args, **kwargs):
        """
        !!! danger ""
            You cannot call this function from a sequencer script.
        """
...
class ThreadComms:
    ...
    * The `state_queue` is used for sequence thread -> main thread communication.
    * All other parameters are used for main thread -> sequence thread communication.

    Attributes:
        pause (bool): user has asked to pause the script, and we
            won't move to the next line until they unpause
        stop (bool): user has asked to stop the script
        debug (bool): user is in debug mode and we shouldn't move to
            the next line in the script until they say "step over:
        stop_over (bool): user is in debug mode and wants to move to
            the next line in the script
        message_acknowledged (bool): the script called [`sequencer_msg`][]
            with "wait=True", and the user has now acknowledged the
            message, so we can move to the next line in the script
        state_queue (queue.Queue): updates of what the sequence thread is doing
...
class WaitInfo:
    ...
    Attributes:
        ...
        started_ms (int): UNIX timestamp if `wait_type` is "Seconds" or "ODBValueStability"
        odb_path (str): path we're watching if `wait_type` is "ODBValue" or "ODBValueStability"
        odb_op (str): condition if `wait_type` is "ODBValue" or "ODBValueStability" (e.g. ">", "==")
        timeout_value (float/int): how long we've been waiting, in seconds
        timeout_limit (float/int): timeout, in seconds
    """
    def __init__(self, wait_type, curr_value, target_value, started_ms=0, odb_path="", odb_op="", timeout_value=0, timeout_limit=0):
        ...
        self.odb_op = odb_op
        self.timeout_value = timeout_value if timeout_value is not None else 0
        self.timeout_limit = timeout_limit if timeout_limit is not None else 0
...
class ParamInfo:
    ...
    Members:
        ...
        comment (str): user-facing comment about the parameter
        default_value (int/float/str/bool etc): default value to show the user
        options (list of str): optional list of preset values
    """
...
class KeepModuleInfo:
    """
    Used by the sequence thread to tell the main thread that certain modules should
    not be deleted/reloaded when starting a script.

    Members:
        module_name (list of str): module names to keep
    """
...
def should_trace(filename, func_name):
    """
    Whether we should trace within this function.
    ...
    Parameters:
        filename (str): raw filename (not full path)
        func_name (str): function we are in
    """
    if filename == "<string>":
        # We're in the user's script. Check if the user has disabled tracing...
        return _user_tracing_enabled
    if filename == "client.py":
        # There are a couple of functions we want to introspect in MidasClient
        # (mostly so we can detect when the user initiated a run transition)
...
def trace_lines(frame, event, arg):
    ...
    Parameters:
        frame (Frame): as supplied by Python tracing tools
        event (str): call/line/return/exception/opcode
        arg (anything): depends on the event type
    """
    if event != "line":
        return
...
class ScriptExecutionThread(threading.Thread):
    ...
    Attributes:
        seq (`SequenceClient`): the main object that user scripts interact with
    ...
    def load(self):
        ...
        for mod in list(sys.modules.keys()):
            ...
            for test_mod in self.keep_modules:
                if mod == test_mod or (isinstance(test_mod, re.Pattern) and test_mod.match(mod) is not None):
                    delete = False
                    break
...
class SequenceRunner:
    ...
    Attributes:
        ...
        seq (`SequenceClient`): main object that user scripts interact with
        ...
        exec_thread (`ScriptExecutionThread` or None): thread that user's script will run in
        keep_modules (list of str): modules that should not be reloaded when the 
            user loads a new script. See `SequenceClient.keep_modules_when_reloading()`
            for details.
    """
    def __init__(self, args):
        ...
        default_state = collections.OrderedDict([
            ...
            ("Timeout value", ctypes.c_float(0)),
            ("Timeout limit", ctypes.c_float(0)),
    ...
    def update_state_in_odb(self, force=False):
        ...
        Parameters:
            force (bool): normally we only update every 1s (as set by 
                `self.odb_update_period`), but you can force an update now.
        """
        ...
        while not comms.state_queue.empty():
            ...
            elif isinstance(msg, KeepModuleInfo):
                for mod in msg.module_names:
                    # Note this check works for both basic strings and `re.Pattern`
                    # objects, as calling `re.compile()` with the same argument
                    # returns the exact same `re.Pattern` object!
                    if mod not in self.keep_modules:
                        self.keep_modules.append(mod)
        ...
        if force or now - self.odb_update_last > self.odb_update_period:
            ...
            if self.latest_wait_info is None:
                ...
                self.set_state_odb("Wait limit", 0)
                self.set_state_odb("Start time", 0)
                self.set_state_odb("Timeout value", 0)
                self.set_state_odb("Timeout limit", 0)
            else:
                curr = self.latest_wait_info.curr_value
                targ = self.latest_wait_info.target_value

                if self.latest_wait_info.wait_type in ["Seconds", "ODBValueStability"]:
                    # msequencer reports in ms rather than secs. Match that behaviour.
                    curr = round(curr * 1000, 0)
                    targ = round(targ * 1000, 0)

                if self.latest_wait_info.wait_type == "ODBValue":
                    wait_type = "ODB" + self.latest_wait_info.odb_op
                else:
                    wait_type = self.latest_wait_info.wait_type

                self.set_state_odb("Wait type", wait_type)
                ...
                self.set_state_odb("Timeout value", self.latest_wait_info.timeout_value)
                self.set_state_odb("Timeout limit", self.latest_wait_info.timeout_limit)
    ...
    def get_odb_repr(self, v):
        """
        Convert a local/global variable into a string that can be shown in the ODB. 
        ...
        Returns:
            (str): string representation, or None if we shouldn't show it
        """
        # We store variables in ODB as strings
        report_types = [int, float, str, bool]

        if have_numpy:
            report_types.extend([np.number, np.bool_])
    ...
    def report_error(self, msg, line):
        ...
        Parameters:
            ...
            line (int): line in the user's script where the error happened
        """
